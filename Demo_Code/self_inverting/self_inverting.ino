 ...
 * Now, let Rocky down into a normal pendulum state - it should start working!
 *
...
const float AX_THRESH = 1; // should be unsigned! Absolute value of upper limit on x-accel
const float AY_THRESH = -9.8; // should be signed! measured g when imu is oriented along the positive vertical
  ...
  if((cur_time - prev_time) > UPDATE_TIME_MS){
    ...
    if(angle_rad > 0.1 || angle_rad < -0.1)      // If angle is not within +- 6 degrees, reset counter that waits for start
    {
      start_counter = 0;
		...
		if(abs(imu.a.x) < AX_THRESH && imu.a.y < AY_THRESH && !start_flag)
		{
